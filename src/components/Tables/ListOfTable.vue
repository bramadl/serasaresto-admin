...
emitter.on('refresh:tables', async () => {
  const { data: { data: tables } } = await useFetch('get', props.endpoint)
  items.value = tables
})
  ...
  <table>
    ...
    <tbody>
      <template v-if="itemsPaginated.length">
        <tr
          v-for="(table, index) in itemsPaginated"
          :key="table.id"
          :class="[tableTrStyle, index % 2 === 0 ? tableTrOddStyle : '']"
        >
          <checkbox-cell
            v-if="checkable"
            @checked="checked($event, table)"
          />
          <td data-label="TableNumber">
            {{ table.number }}
          </td>
          <td data-label="TableToken">
            {{ trimTo(20, table.token) }}
          </td>
          <td data-label="IsReserved">
            {{ table.isReserved ? "Direservasi" : "Kosong" }}
          </td>
          <td data-label="TableCustomer">
            {{ table.customer[0] ? table.customer[0].name : '-' }}
          </td>
          <td data-label="TableOrders">
